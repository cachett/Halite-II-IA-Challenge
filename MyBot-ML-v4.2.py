...
while True:
    ...
    for p in game_map.all_planets():
        ...
        planet_ship_docking[p.id] = p.num_docking_spots - len(p._docked_ship_ids)
            ...
            try:
                ...
                # Mine
                    ...
                    if len(closest_opendock_planets) > 0:
                        i = 0
                        while i < len(closest_opendock_planets) and planet_ship_docking[closest_opendock_planets[i][1].id] <= 0:
                            i += 1
                        if i == len(closest_opendock_planets):
                            target = closest_enemy_ships[0][1]
                        else:
                            target = closest_opendock_planets[i][1]
                            planet_ship_docking[target.id] -= 1
